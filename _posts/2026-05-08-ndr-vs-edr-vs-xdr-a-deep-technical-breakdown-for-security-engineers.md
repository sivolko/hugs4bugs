...
tags:
  ...
  - soc
subtitle: ""
description: "Most teams deploy EDR and call it done. This breakdown covers what NDR, EDR, and XDR each actually see, where they go blind without each other, and how to stack them based on your SOC's maturity."
...
NDR operates at the network layer. Sensors are deployed out-of-bandâtypically via SPAN ports, TAPs (Test Access Points), or virtual mirrors in cloud environmentsâand passively observe raw traffic without introducing latency into the network path. This gives NDR a unique vantage point: it sees every device on the network, including those that cannot run software agents, such as IoT devices, OT/ICS equipment, IP phones, and legacy hardware.
...
- **East/west (lateral) traffic**: communication between devices within the same internal networkâthe movement pattern most associated with post-compromise adversary behavior.

### How It Works Technically

NDR captures and parses raw packets or flow records, then applies a layered detection stack:

1. **Protocol dissection**: Deep packet inspection extracts metadata from protocols (DNS, HTTP/S, TLS, SMB, Kerberos, RDP, etc.). Platforms like Corelight use ZeekÂ® to convert this traffic into structured, richly correlated logsâfar more granular than NetFlow or firewall logs.

2. **Signature-based detection**: Frameworks like SuricataÂ® IDS match traffic against known-bad patterns (malware C2 signatures, exploit payloads, etc.).

3. **Behavioral/anomaly detection**: Machine learning builds baselines of normal traffic patterns, then flags deviationsâunusual port usage, anomalous transfer volumes, odd timing, malformed packets, unauthorized protocols.
...
Smart PCAP implementations extend forensic value by capturing only targeted packet subsets based on alert triggers, protocol type, or encryption statusâbalancing storage cost against investigative fidelity.
...
```
     ...
     [Firewall / Gateway]
      ...
      [SPAN Port / TAP]
            |
      [NDR Sensor]  âââââââââââââââââââââââââââââ
            |                                     |
...
| | |
|---|---|
| **Benefit** | Agentlessâcovers every device, including unmanageable endpoints |
...
EDR operates at the host level. A software agent is installed on each managed endpointâlaptops, desktops, servers, workstationsâand continuously monitors all activity occurring on that device. This gives EDR a level of process-level and user-level granularity that network-layer tools cannot reach.
...
| **Limitation** | Requires agent deploymentâcannot cover agentless devices (IoT, OT, printers) |
...
2. **Cross-domain correlation**: Alerts and raw events are correlated across sources. Example: a phishing email detection (email gateway) â a PowerShell execution on the recipient's endpoint (EDR) â an outbound DNS query to an unknown domain (NDR) â a cloud storage upload (CASB) can be stitched into a single incident timeline.
...
4. **Automated response**: Coordinated response actions can fire across domains simultaneouslyâquarantine the email, isolate the endpoint, block the IP at the firewall, and suspend the user account.
...
```
Phishing Email â Malware on Endpoint â Lateral Movement â Cloud Exfiltration
     ^                   ^                    ^                  ^
...
| **Limitation** | Marketing hype is significantâevaluation requires rigor |
...
```
                  ...
                  â    SIEM     â  â Log aggregation, compliance, long-term retention
                  â  (or XDR)  â  â Cross-domain correlation, automated response
                  ââââââââ¬âââââââ
                         â
    ...
    â     NDR     â             â     EDR     â
    â  (Network)  â             â  (Endpoint) â
    âââââââââââââââ             âââââââââââââââ

```

- **NDR** provides the network visibility layerâthe only layer that sees east/west lateral movement, agentless devices, and network-based C2.
- **EDR** provides the endpoint visibility layerâthe only layer that sees process-level execution, memory operations, and user behavior on managed devices.
- **SIEM** (or XDR) provides the aggregation and correlation layerâingesting logs and alerts from NDR, EDR, and other sources, applying analytics, and providing the SOC's central operations hub.
...
Some practitioners, notably Jean Schaffer (former CISO of the Defense Intelligence Agency), propose expanding the triad to a **quintet** by adding cloud and identity as independent visibility pillarsârecognizing that neither NDR nor EDR provides complete coverage of cloud-native workloads or identity-based attack paths.
...
- An NDR alert fires on an anomalous outbound TLS connection. The destination IP is unknown. EDR context reveals which process on which user's machine initiated the connectionâimmediately narrowing the investigation.
- An EDR alert fires on a suspicious PowerShell execution. NDR context reveals whether that host subsequently made outbound connections to unusual destinationsâconfirming or ruling out active C2.
...
| 100â1,000 endpoints, small SOC | EDR + NDR |
...
**For XDR:**
- Native or open architectureâwhat are the integration constraints?
...
- **NDR** is the only tool that sees every device on the network, east/west lateral movement, and network-level attacker behaviorâwithout requiring agent deployment.
...
A security program that deploys only EDR is blind to what happens on the network. One that deploys only NDR is blind to what happens on the host. XDR without high-quality NDR and EDR data sources is just a dashboard with low-fidelity inputs. The triad works because the visibility domains genuinely do not overlapâand adversaries have always exploited the seams between security tools. Closing those seams is the architectural goal.